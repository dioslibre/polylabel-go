...
func boundingBox(polygon [][][]float64) (minX float64, minY float64, maxX float64, maxY float64) {
	coords := polygon[0]
...
func pointToPolygonDistance(x float64, y float64, polygon [][][]float64) float64 {
	inside := false
...
func segmentDistanceSquared(px float64, py float64, a []float64, b []float64) float64 {
	x := a[0]
	y := a[1]
	dx := b[0] - x
	dy := b[1] - y
